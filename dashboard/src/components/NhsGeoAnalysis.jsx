...
function PieTooltip({ active, payload }) {
  ...
  return (
    <div style={{ background: 'white', border: '1px solid #e0e4ea', borderRadius: 6, padding: '10px 14px', fontSize: 13 }}>
      ...
      <div>{fmt(d.procedures)} NHS procedures</div>
      <div style={{ color: '#6b7a90' }}>{d.marketSharePct}% local share</div>
    </div>
  );
}

export default function NhsGeoAnalysis({ nhsSiteProcIndex, nhsGroupSiteData }) {
  const [selectedSiteId, setSelectedSiteId] = useState('');
  const [radiusMiles, setRadiusMiles] = useState(50);
  const [procSearch, setProcSearch] = useState('');
  const [drillProc, setDrillProc] = useState(null);
  const [siteSearch, setSiteSearch] = useState('');

  // Nuffield sites with coordinates (from NHS group summary)
  const nuffieldSites = useMemo(() => {
    if (!nhsGroupSiteData?.Nuffield?.sites) return [];
    return nhsGroupSiteData.Nuffield.sites
  ...
  if (!nhsSiteProcIndex || !nhsGroupSiteData) {
    return <div className="loading">Loading NHS geographic data…</div>;
  }

  return (
    <>
      {/* Explanatory note */}
      <div style={{ background: '#f0f4f8', border: '1px solid #d0d7e2', borderRadius: 8, padding: '10px 16px', marginBottom: 16, fontSize: 13, color: '#4a5568' }}>
        NHS-commissioned procedures performed at private hospital sites (Nuffield, Spire, Circle, Ramsay).
        Shows the NHS insourcing / waiting list partnership market within a selected catchment.
      </div>
      ...
      {!selectedSite && (
        <div style={{ textAlign: 'center', color: '#6b7a90', padding: '60px 0', fontSize: 15 }}>
          Select a Nuffield site to see the local NHS-commissioned market.
        </div>
      )}

      {selectedSite && !procedureTable.length && (
        <div style={{ textAlign: 'center', color: '#6b7a90', padding: '60px 0' }}>
          No NHS procedure data found within {radiusMiles} miles of {selectedSite.siteName}.
        </div>
      )}
            ...
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #e0e4ea' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div className="card-title" style={{ marginBottom: 0 }}>
                  NHS Procedure Volume &amp; Nuffield Share — within {radiusMiles} miles
                </div>
                {drillProc && (
                  <button
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: 'right' }}>NHS Volume</th>
          ...
          {drillProc && drillData && (
            <>
              <div className="charts-row" style={{ marginBottom: 20 }}>
                <div className="card">
                  <div className="card-title">{drillProc} — local NHS share ({radiusMiles}mi)</div>
                ...
                <div className="card">
                  <div className="card-title">NHS procedures by group</div>
                  <ResponsiveContainer width="100%" height={240}>
                    <BarChart
              ...
              <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #e0e4ea' }}>
                  <div className="card-title" style={{ marginBottom: 10 }}>
                    Competing sites — {drillProc} — within {radiusMiles} miles ({drillData.competitors.length} sites, {fmt(drillData.total)} total NHS procedures)
                  </div>
                  <div className="filter-row" style={{ marginBottom: 0 }}>
                    <input
                      type="text"
                  ...
                  <table className="data-table">
                    <thead>
                      <tr>
                        ...
                        <th style={{ textAlign: 'right' }}>NHS Procedures</th>
